...
export default function Dashboard() {
  ...
  const filteredInvoices = useMemo(() => {
    return invoices.filter(
      (i) => new Date(i.createdAt).getTime() >= filterDate
    );
  }, [invoices, filterDate]);
    ...
    if (filter === "daily") {
      ...
      for (let i = days - 1; i >= 0; i--) {
        ...
        const dailyIncome = invoices
          .filter(
            (inv) =>
              inv.createdAt.startsWith(isoDate) &&
  ...
  if (company.id === "default") return null;
  return (
          ...
          <div className="space-y-4">
            {invoices
              ...
              .map((inv) => (
                <div
                  key={inv.id}
                  className="flex justify-between items-center p-3 hover:bg-slate-50 rounded-lg transition-colors border-b border-slate-50 last:border-0"
                >
