...
export default function Expenses() {
  ...
  const [form, setForm] = useState({ title: '', amount: '', date: new Date().toISOString().split('T')[0], vendorId: '' });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setForm({ title: '', amount: '', date: new Date().toISOString().split('T')[0], vendorId: '' });
  };
                ...
                {expenses.slice().reverse().map(e => (
                  <tr key={e.id} className="hover:bg-slate-50">
                    ...
                    <td className="p-4 text-slate-600 text-sm">
                      {e.vendorName || <span className="text-slate-400 italic">N/A</span>}
                    </td>
                    <td className="p-4 text-slate-500 text-sm flex items-center gap-2">
                      <Calendar size={14} /> {new Date(e.date).toLocaleDateString()}
                    </td>
                    <td className="p-4 text-right font-medium text-rose-600">- {company.currency} {e.amount.toFixed(2)}</td>
                    <td className="p-4 text-center">
                      <button onClick={() => deleteExpense(e.id)} className="text-slate-400 hover:text-red-600 transition-colors">
                        <Trash2 size={18} />
                      </button>
                    </td>
                  </tr>
